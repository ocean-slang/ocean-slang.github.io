---
layout: page
title: NASA S-MODE Bio-optical Calibrations
description: 
img: assets/img/smode.png
importance: 5
category: fun
---

<h1>[S-MODE DATA](https://podaac.jpl.nasa.gov/S-MODE)</h1>

<h1><a href="https://archive.podaac.earthdata.nasa.gov/podaac-ops-cumulus-docs/s-mode/docs/SMODE_PILOT_Bio-optical_Cals_V3.pdf">S-MODE PILOT BIO-OPTICAL REPORT</a></h1>


<div class="row">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/calpilot1.png" title="cals" class="img-fluid rounded z-depth-1" %}
    </div>
</div>

<h1><a href="https://archive.podaac.earthdata.nasa.gov/podaac-ops-cumulus-docs/s-mode/docs/S-MODE_IOP1_2022_BIO_OPTICS_DATA_REPORT.pdf">S-MODE IOP-1 BIO-OPTICAL REPORT</a></h1>

<div class="row">
  <div class="column">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/caliop1b.png" title="cals" class="img-fluid rounded z-depth-1" %}
    </div>
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/caliop1.png" title="cals" class="img-fluid rounded z-depth-1" %}
    </div>
</div>

<h1><a href="https://archive.podaac.earthdata.nasa.gov/podaac-ops-cumulus-docs/s-mode/docs/SMODE_IOP2_Bio-optical_Cals_V1.pdf">S-MODE IOP2 BIO-OPTICAL REPORT</a></h1>

<div class="row">
  <div class="column">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/caliop2b.png" title="cals" class="img-fluid rounded z-depth-1" %}
    </div>
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/caliop2a.png" title="cals" class="img-fluid rounded z-depth-1" %}
    </div>
</div>

